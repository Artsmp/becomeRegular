...
const formatter = computed(
  () => (currentRoute.value?.meta?.slide as any)?.frontmatter || {}
);
...
useEventListener('click', (e) => {
  if (formatter.value.growFollow != null) return;
...
const left = computed(() =>
  followMouse.value ? mouseX.value : formatter.value.growX ?? '80'
);
const top = computed(() =>
  followMouse.value ? mouseY.value : formatter.value.growY ?? '30'
);
